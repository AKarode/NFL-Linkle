...
def get_file_path(year, team_abbreviation):
    ...
    file_path = current_dir + "yearly_roster_data/" + file_name
    return file_path
    ...
    while retry_count > 0:
        ...
        if response.status_code in (400, 404):  # Indicates a bad request
            ...
            raise requests.exceptions.RequestException(
                "Unknown error with " f"status code {response.status_code} encountered"
            )
...
def download_yearly_roster_data(year, team, file_path):
    ...
    try:
        ...
        raise RuntimeError(f"Encountered {e} when making a URL request")
